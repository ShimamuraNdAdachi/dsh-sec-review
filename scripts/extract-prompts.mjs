...
const document = {
  ...
  // v3 only: upstream ships the AGPL-3 text without an "or any later version"
  // grant, so a derivative may not offer recipients a later version.
  license: 'AGPL-3.0-only (upstream: protectai/vulnhuntr)',
  ...
  // Counts of what was reproduced, so THIRD_PARTY_NOTICES.md states figures that
  // are checkable straight off this file instead of numbers nobody can verify.
  stats: {
    classTemplates: Object.keys(parsed.templates).length,
    classTemplateChars: Object.values(parsed.templates).reduce((n, text) => n + text.length, 0),
    sharedPrompts: SHARED_TEMPLATES.length,
    sharedPromptChars: Object.values(shared).reduce((n, text) => n + text.length, 0),
    bypassStrings: Object.values(parsed.bypasses).reduce((n, list) => n + list.length, 0),
  },
}
